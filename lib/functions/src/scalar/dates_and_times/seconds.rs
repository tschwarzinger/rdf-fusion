...
pub fn seconds_udf(
    ...
    Ok(ScalarUDF::new_from_impl(SecondsSparqlUDF::new(encodings)))
}

#[derive(Clone, PartialEq, Eq, Hash)]
struct SecondsSparqlUDF {
    encodings: RdfFusionEncodings,
    name: String,
    signature: Signature,
}

impl Debug for SecondsSparqlUDF {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SecondsSparqlUDF")
            .field("encodings", &self.encodings)
            .finish()
    }
}

impl SecondsSparqlUDF {
    /// Create a new [`SecondsSparqlUDF`].
