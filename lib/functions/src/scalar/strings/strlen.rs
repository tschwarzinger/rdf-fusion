...
pub fn strlen_udf(
    ...
    Ok(ScalarUDF::new_from_impl(StrLenSparqlUDF::new(encodings)))
}

#[derive(Clone, PartialEq, Eq, Hash)]
struct StrLenSparqlUDF {
    encodings: RdfFusionEncodings,
    name: String,
    signature: Signature,
}

impl Debug for StrLenSparqlUDF {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StrLenSparqlUDF")
            .field("encodings", &self.encodings)
            .finish()
    }
}

impl StrLenSparqlUDF {
    /// Create a new [`StrLenSparqlUDF`].
...
impl ScalarUDFImpl for StrLenSparqlUDF {
