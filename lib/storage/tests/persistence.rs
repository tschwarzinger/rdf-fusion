...
use deltalake::logstore::{LogStore, StorageConfig, logstore_with};
...
async fn test_reload_storage_plain_term() {
    ...
    // 2. Reload and verify
    {
        ...
        let storage = DeltaQuadStorage::try_load(&ctx.state(), Arc::clone(&log_store))
            .await
            .unwrap();
...
async fn test_reload_storage_with_index_and_optimize() {
    ...
    // 2. Reload, add more data, and optimize again
    {
        let ctx = SessionContext::new();
        let storage = Arc::new(
            DeltaQuadStorage::try_load(&ctx.state(), Arc::clone(&log_store))
                .await
                .unwrap(),
        );
