...
use rdf_fusion_common::{QuadComponent, RdfSortOrder};
use rdf_fusion_encoding::QuadStorageEncoding;

/// TODO: Make this configurable
pub const PAGE_ROW_COUNT: usize = 1_024;
/// TODO: Make this configurable
pub const ROW_GROUP_ROW_COUNT: usize = PAGE_ROW_COUNT * 256;
/// TODO: Make this configurable
...
impl RdfFusionParquetWriterProperties {
    ...
    pub fn to_arrow(&self) -> WriterProperties {
        let sorting_columns = self.sort_order.as_ref().and_then(|order| match order {
            RdfSortOrder::NativeOrder(order) => {
                if order.is_empty() {
                    None
                } else {
                    Some(
        ...
        let non_clustered_columns =
            self.sort_order.as_ref().and_then(|order| match order {
                RdfSortOrder::NativeOrder(order) => {
        ...
        for non_clustered_column in non_clustered_columns.iter().flatten() {
            builder = if self.encoding.term_type().is_primitive() {
