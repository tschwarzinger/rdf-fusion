...
static CLAIM: LazyLock<TypeClaim> = LazyLock::new(|| TypeClaim::Iri);

impl TypedFamily for IriFamily {
    type Array = IriFamilyArray;

    const FAMILY_ID: TypedFamilyId = TypedFamilyId::Iri;
    ...
    fn create_array_from_plain_term(array: &PlainTermArray) -> AResult<IriFamilyArray> {
        ...
        return Ok(IriFamilyArray::from_array_unchecked(Arc::new(
            builder.finish(),
        )));
...
impl IriFamily {
    pub fn create_array(array: StringArray) -> AResult<IriFamilyArray> {
        Ok(IriFamilyArray::new(array))
    }
}

/// A family-specific array for the [`IriFamily`].
#[derive(Debug, Clone)]
pub struct IriFamilyArray {
    array: ArrayRef,
}

impl IriFamilyArray {
    /// Creates a new [`IriFamilyArray`].
...
impl FamilyArray for IriFamilyArray {
    type Family = IriFamily;
