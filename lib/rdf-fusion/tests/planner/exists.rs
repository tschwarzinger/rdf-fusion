    ...
    assert_query_explanation(store, query, |explanation| {
        ...
        DecodeObjectIdsExec: projections=[decode(s) as s]
          HashJoinExec: mode=CollectLeft, join_type=LeftAnti, on=[(s@0, __inner__s@0)]
            ...
            ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?s <http://example.org/p1> <http://example.org/o2>], blank_node_mode=Variable, file_groups={1 group: [[]]}, projection=[subject@1 as __inner__s], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 1 AND object@3 = 3, pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 1 AND 1 <= predicate_max@2 AND object_null_count@7 != row_count@4 AND object_min@5 <= 3 AND 3 <= object_max@6, required_guarantees=[object in (3), predicate in (1)]
        ");
    }).await;
}
