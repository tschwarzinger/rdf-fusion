    ...
    assert_query_explanation(store, query, |explanation| {
        ...
        DecodeObjectIdsExec: projections=[decode(s__oid) as s]
          HashJoinExec: mode=CollectLeft, join_type=Inner, on=[(s__oid@0, s__oid@0)], projection=[s__oid@0]
            ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?s <http://example.org/p2> ?o2], blank_node_mode=Variable, file_groups={1 group: [[GPOS/<name>.parquet]]}, projection=[subject@1 as s__oid], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 3, pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 3 AND 3 <= predicate_max@2, required_guarantees=[predicate in (3)]
            ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?s <http://example.org/p1> <http://example.org/o1>], blank_node_mode=Variable, file_groups={1 group: [[GPOS/<name>.parquet]]}, projection=[subject@1 as s__oid], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 1 AND object@3 = 2 AND DynamicFilter [ empty ], pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 1 AND 1 <= predicate_max@2 AND object_null_count@7 != row_count@4 AND object_min@5 <= 2 AND 2 <= object_max@6, required_guarantees=[object in (2), predicate in (1)]
        ");
    }).await;
}
    ...
    assert_query_explanation(store, query, |explanation| {
        ...
        DecodeObjectIdsExec: projections=[decode(s__oid) as s]
          HashJoinExec: mode=CollectLeft, join_type=Inner, on=[(o1__oid@0, o1__oid@1)], projection=[s__oid@1]
            ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?o1 <http://example.org/p2> ?o2], blank_node_mode=Variable, file_groups={1 group: [[GPOS/<name>.parquet]]}, projection=[subject@1 as o1__oid], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 3, pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 3 AND 3 <= predicate_max@2, required_guarantees=[predicate in (3)]
            ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?s <http://example.org/p1> ?o1], blank_node_mode=Variable, file_groups={1 group: [[GPOS/<name>.parquet]]}, projection=[subject@1 as s__oid, object@3 as o1__oid], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 1 AND DynamicFilter [ empty ], pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 1 AND 1 <= predicate_max@2, required_guarantees=[predicate in (1)]
        ");
    }).await;
}
        ...
        FilterExec: EBV(EQ(ENC_TF(STR(name@2)), 2:{value:Paris,language:})) AND EBV(EQ(ENC_TF(STR(type@1)), 2:{value:http://schema.org/City,language:})) AND EBV(EQ(ENC_TF(STR(city@0)), 2:{value:http://www.wikidata.org/entity/Q90,language:})), projection=[city@0, name@2]
          DecodeObjectIdsExec: projections=[decode(city__oid) as city, decode(type__oid) as type, decode(name__oid) as name]
            HashJoinExec: mode=CollectLeft, join_type=Inner, on=[(city__oid@0, city__oid@0)], projection=[city__oid@0, type__oid@1, name__oid@3]
              ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?city <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> ?type], blank_node_mode=Variable, file_groups={1 group: [[GPOS/<name>.parquet]]}, projection=[subject@1 as city__oid, object@3 as type__oid], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 4, pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 4 AND 4 <= predicate_max@2, required_guarantees=[predicate in (4)]
              ParquetQuadScanExec: active_graph=Default Graph, triple_pattern=[?city <http://schema.org/name> ?name], blank_node_mode=Variable, file_groups={1 group: [[GPOS/<name>.parquet]]}, projection=[subject@1 as city__oid, object@3 as name__oid], file_type=parquet, predicate=graph@0 IS NULL AND predicate@2 = 3 AND DynamicFilter [ empty ], pruning_predicate=graph_null_count@0 > 0 AND predicate_null_count@3 != row_count@4 AND predicate_min@1 <= 3 AND 3 <= predicate_max@2, required_guarantees=[predicate in (3)]
        ");
    }).await;
}
